...
export class CreateDoctorScheduleDto {
  @ApiProperty({ description: 'ID of the doctor', required: true })
  @IsNotEmpty()
  @IsUUID()
  doctorId: string;
  ...
  @ApiProperty({ example: '09:00', description: 'Start time in HH:MM format (24-hour)' })
  @IsNotEmpty()
  @IsString()
  @Matches(/^([01]\d|2[0-3]):([0-5]\d)$/, { message: 'startTime must be in HH:MM format' })
  startTime: string;

  @ApiProperty({ example: '17:00', description: 'End time in HH:MM format (24-hour)' })
  @IsNotEmpty()
  @IsString()
